...
static int stextlrpad = 8;   /* sum of left and right padding for status text */
static int bvpad = 4;        /* bar vertical padding */
...
{
	(void)memcpy(m->ltsymbol, m->lt[m->sellt]->symbol, sizeof(m->ltsymbol));
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - (int)TEXTW(stext) + lrpad - stextlrpad)
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext) - lrpad + stextlrpad;
		drw_text(drw, m->ww - tw, 0, tw, bh, stextlrpad/2, stext, 0);
	}
...
setlayout(const Arg *arg)
{
	...
	(void)memcpy(selmon->ltsymbol, selmon->lt[selmon->sellt]->symbol, sizeof(selmon->ltsymbol));
	if (selmon->sel)
		arrange(selmon);
	else
		drawbar(selmon);
